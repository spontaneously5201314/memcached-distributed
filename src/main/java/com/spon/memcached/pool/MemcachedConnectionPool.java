...
public class MemcachedConnectionPool {
    ...
    private NodeManagerFactory factory;

    private MemcachedConnectionPool() {
    }

    public void setFactory(NodeManagerFactory factory) {
    ...
    public void init() throws IOException {
        ...
        //这里可以把所有的节点都连接上，并存储进去connections中
        List<MemcachedNode> allNodes = factory.getAllNodes();
        for (MemcachedNode node : allNodes) {
            StringBuilder sb = new StringBuilder();
            sb.append(node.getAddress()).append(":").append(node.getPort());
            connetions.put(factory.getUsedAlgorithm().hash(sb.toString()), new MemcachedClient(new InetSocketAddress(node.getAddress(), node.getPort())));
        }
